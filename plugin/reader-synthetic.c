...
LOCAL thread_ctx_t  synCtx;         /* generator context (config/rng/stats)    */

LOCAL uint64_t      maxSessions = 100;  /* 0 = unbounded                       */
LOCAL uint64_t      maxPackets  = 0;    /* 0 = unbounded                       */
...
LOCAL uint64_t      synClockUS;         /* running synthetic clock             */

LOCAL uint64_t      synPackets;
LOCAL uint64_t      synDropped;
LOCAL uint64_t      synSessions;
...
LOCAL struct timeval synthetic_next_ts(void)
{
    struct timeval tv;
    tv.tv_sec  = synClockUS / 1000000ULL;
    tv.tv_usec = synClockUS % 1000000ULL;
    synClockUS += timeDeltaUS;
    return tv;
}
...
LOCAL void synthetic_parse_cmdline(void)
{
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "i:g:t:d:G:vm:S:D:P:s:M:b:T:f",
                              long_opts, NULL)) != -1) {
        switch (opt) {
        case 'm':
    ...
    synClockUS = baseTimeUS;
    rng_seed(synCtx.rng_state, seed);
}

/* ---- reader thread --------------------------------------------------------*/
LOCAL void *synthetic_thread(gpointer UNUSED(uw))
{
    ...
    while (!config.quitting) {
        if (!runForever) {
            if (maxSessions && synSessions >= maxSessions) break;
            if (maxPackets  && synPackets  >= maxPackets)  break;
        }

        if (need_new) {
            protocol_t proto = pick_protocol(&synCtx);
            session_init(&session, proto, &synCtx);
            synSessions++;
            need_new = 0;
        }

        int len = session_step(&session, buf, &synCtx);
        if (len == 0) { need_new = 1; continue; }  /* session complete         */
        if (len < 0)  { synDropped++; continue; }  /* simulated drop           */
    ...
    /* Input exhausted: tell capture to drain sessions to ES and exit. */
    if (!runForever)
        arkime_quit();
...
{
    /* Single generator thread => deterministic emission order. */
    g_thread_unref(g_thread_new("reader-synthetic", &synthetic_thread, NULL));
}

LOCAL int synthetic_stats(ArkimeReaderStats_t *stats)
{
    stats->total   = synPackets;
    stats->dropped = synDropped;
    return 0;
}
